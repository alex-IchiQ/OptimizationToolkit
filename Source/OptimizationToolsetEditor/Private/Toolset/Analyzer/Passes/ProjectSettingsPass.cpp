...
void FProjectSettingsPass::Run(const FLevelScanContext& Context, const FAnalyzeThresholds& T, FScanResult& Out) const
{
	...
	{
		FFinding F(TEXT("Project.OcclusionCullingDisabled"), ESeverity::Critical, ECategory::Project, EFindingScope::Project, LOCTEXT("OcclusionTitle", "Occlusion culling is disabled"), Subject);
	...
	{
		FFinding F(TEXT("Project.StaticLightingWithLumen"), ESeverity::Major, ECategory::Project, EFindingScope::Project, LOCTEXT("StaticLightingTitle", "Static lighting is allowed while Lumen provides GI"), Subject);
	...
	{
		FFinding F(TEXT("Project.RayTracingEnabled"), ESeverity::Minor, ECategory::Project, EFindingScope::Project, LOCTEXT("RayTracingTitle", "Hardware ray tracing is enabled"), Subject);
		...
		// UE 5.8 replaced bSupportSkinCacheShaders with a three-state enum so
		// projects can compile inline shaders, skin-cache shaders, or both.
#if OPTIMIZATION_HAS_SKINNING_SHADER_SUPPORT_MODE
		const bool bSkinCacheShadersEnabled = Settings->SkinningShaderSupport != ESkinningShaderSupportMode::InlineShadersOnly;
#else
		const bool bSkinCacheShadersEnabled = Settings->bSupportSkinCacheShaders;
#endif
		if (!bSkinCacheShadersEnabled)
		{
			FFinding G(TEXT("Project.RayTracingWithoutSkinCache"), ESeverity::Major, ECategory::Project, EFindingScope::Project, LOCTEXT("SkinCacheTitle", "Ray tracing is enabled without skin cache shaders"), Subject);
